...
/*	printf("%d %d\n", size.x, size.y);
	...
	h = (unsigned long *)calloc(size.max + 1, sizeof(unsigned long));

	for (count = 0; count < size.x * size.y; count++)
		h[(maps[0])[count].gray]++;

	for (count = 0; count < size.max + 1; count++)
		if (h[count] != 0) H -= log((double)h[count] / size.x / size.y) / log(2) * h[count] / size.x / size.y;
