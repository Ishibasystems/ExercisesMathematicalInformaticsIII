...
void main(void)
{
	...
	for (count = 0; count < size.x * size.y; count++) Tei1[count] = maps[count].gray;
	...
	Tei2[0] = Tei1[0];
	for (count = 1; count < size.x * size.y; count++)
		Tei2[count] = (Tei1[count - 1] - Tei1[count]) + 255;

	printf("予測符号化\nENTROPY: %lf\n", entropy(Tei2, size.x * size.y, size.max + 256));
